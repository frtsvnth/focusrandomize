...
const BASE_ARENA_LEFT = 40;
const BASE_ARENA_RIGHT = 760;
...
function StickmanAdapterV2({ teams, targetTeam, reducedMotion, onComplete, sound }: MechanicAdapterV2Props) {
  ...
  useEffect(() => {
    ...
    class Stickman {
      ...
      update(dt: number) {
        ...
        if (this.state === 'attack') {
          ...
          if (this.stateTimer > 0.15 && this.stateTimer < 0.35 && dist < 60 * scale) {
            ...
            if (dodgeRoll > this.target.dodgeChance || timeSinceDodge < 1500) {
              ...
              spawnTextParticle(this.target.x, this.target.y - 70 * scale, 'MISS!', '#fff');
          ...
          this.state = 'walk';
          if (dist > FIGHT_CONFIG.combatApproachDist * scale) {
            this.x += this.facing * this.speed * 60 * dt * scale;
          } else if (dist < FIGHT_CONFIG.combatRetreatDist * scale) {
            this.x -= this.facing * this.speed * 35 * dt * scale;
          } else if (this.attackCooldown <= 0) {
            this.attack();
          }
        }
        this.x = Math.max(ARENA_LEFT + 15 * scale, Math.min(ARENA_RIGHT - 15 * scale, this.x));
      }
      ...
      takeDamage(amount: number, attacker: Stickman) {
        ...
        this.x -= attacker.facing * 28 * scale;
        spawnHitParticles(this.x, this.y - 35 * scale, this.color);
        const now = performance.now();
        if (now - lastClang > 90) {
          lastClang = now;
          sound.playClunk(1.8);
        ...
        if (this.hp <= 0) {
          ...
          this.state = 'dead';
          this.deathAnim = 0;
          spawnDeathParticles(this.x, this.y - 30 * scale, this.color);
          sound.playHoofbeat(0.6);
      ...
      draw() {
        ...
        const bodyLen = 32 * scale;
        const limbLen = 24 * scale;
        const restAngle = 0.35;
        let strikeArmLen = limbLen;
        let strikeThick = 4.5 * scale;
        ...
        if (this.state === 'walk' && this.alive) {
          ...
          bobY = Math.sin(this.animPhase) * 6 * scale;
        ...
        ctx.fillStyle = '#111';
        const eyeOff = this.facing * 3 * scale;
        ctx.beginPath();
        ctx.arc(eyeOff - 3 * scale, -bodyLen - headR - 2 * scale, 2 * scale, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(eyeOff + 3 * scale, -bodyLen - headR - 2 * scale, 2 * scale, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        if (this.state === 'hit') ctx.arc(eyeOff, -bodyLen - headR + 4 * scale, 3 * scale, Math.PI, 0);
        else if (this.state === 'victory') ctx.arc(eyeOff, -bodyLen - headR + 2 * scale, 4 * scale, 0, Math.PI);
        else ctx.arc(eyeOff, -bodyLen - headR + 3 * scale, 2 * scale, 0, Math.PI);
        ctx.stroke();
        ...
        ctx.lineWidth = 4.5 * scale;
        const shoulderY = -bodyLen + 8 * scale;
        ...
        ctx.rotate(-bodyLean);
        ctx.globalAlpha = alpha;
        ctx.font = `bold ${15 * scale}px Inter, system-ui, -apple-system, Segoe UI, Roboto, sans-serif`;
        ctx.fillStyle = '#fff';
        ctx.textAlign = 'center';
        ctx.shadowColor = '#000';
        ctx.shadowBlur = 4;
        ctx.fillText(this.name, 0, -bodyLen - headR - 35 * scale);
        ctx.shadowBlur = 0;
        ...
        if (this.alive) {
          ...
          ctx.fillStyle = '#222';
          ctx.fillRect(-barW / 2, -bodyLen - headR - 25 * scale, barW, barH);
          ctx.fillStyle = hpRatio > 0.5 ? '#22c55e' : hpRatio > 0.25 ? '#eab308' : '#ef4444';
          ctx.fillRect(-barW / 2, -bodyLen - headR - 25 * scale, barW * hpRatio, barH);
        }
        ctx.restore();
      }
    }
    ...
    function spawnHitParticles(x: number, y: number, color: string) {
      for (let i = 0; i < 10; i++)
        particles.push({
          ...
          life: 1, color, size: (3 + Math.random() * 4) * scale, type: 'hit',
        });
      spawnTextParticle(x, y - 25 * scale, `-${Math.floor(Math.random() * 8 + 8)}`, '#ff5555');
    }
    ...
    function spawnDeathParticles(x: number, y: number, color: string) {
      for (let i = 0; i < 25; i++)
        particles.push({
          ...
          vy: (Math.random() - 1) * 9,
          life: 1.2, color, size: (4 + Math.random() * 6) * scale, type: 'death',
        });
      spawnTextParticle(x, y - 40 * scale, '💀', '#fff');
    }

    function spawnTextParticle(x: number, y: number, text: string, color: string) {
      particles.push({ x, y, vx: 0, vy: -2, life: 1.2, color, text, size: 18 * scale, type: 'text' });
    }
    ...
    function loop(now: number) {
      const dt = Math.min(0.033, lastTime ? (now - lastTime) / 1000 : 1 / 60);
      lastTime = now;
      shake.update(dt);
      drawArena();
        ...
        if (!currentWinner && stickmen.length > 1) {
          if (aliveCount === 0) {
            ...
            if (target) {
              ...
              target.state = 'idle';
              target.deathAnim = 0;
              spawnTextParticle(target.x, target.y - 70 * scale, '💪 ЕЩЁ!', '#ffdd00');
            }
      ...
      ctx.save();
      const off = shake.offset(9);
      ctx.translate(off.x, off.y);

      const sorted = [...stickmen].sort((a, b) => (a.alive ? 1 : 0) - (b.alive ? 1 : 0));
      ...
      for (const s of stickmen) {
        if (s.alive) {
          ...
          ctx.fillStyle = '#000';
          ctx.beginPath();
          ctx.ellipse(s.x, GROUND_Y + 2 * scale, 16 * scale, 5 * scale, 0, 0, Math.PI * 2);
          ctx.fill();
          ctx.globalAlpha = 1;
        }
      }
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 16,
        overflow: 'hidden',
        boxShadow: '0 0 50px rgba(233,69,96,0.18)',
      }}
    >
      <canvas ref={canvasRef} style={{ display: 'block', width: '100%', height: '100%', background: '#16213e' }} />
    </div>
  );
}

export default memo(StickmanAdapterV2);
